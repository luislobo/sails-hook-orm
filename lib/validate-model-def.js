...
module.exports = function validateModelDef (originalModelDef, modelIdentity, hook, sails) {
  ...
  normalizedModelDef.native = function (done){
    ...
    // In `sails-mongo` 1.x, the db connection manager is actually
    // the same as the Mongo client's `db` instance
    // but in `sails-mongo` 2.x, it's the actual MongoClient
    var db = normalizedModelDef.getDatastore().manager.constructor.name === 'MongoClient' ?
      normalizedModelDef.getDatastore().manager._sailsDbInstance :
      normalizedModelDef.getDatastore().manager;
